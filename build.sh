...
name="$(jq -r ".name" manifest.json)-$(jq -r ".version" manifest.json)"
npx web-ext build --ignore-files="assets/**" --overwrite-dest --filename="$name.zip"

echo ""
echo "> Build for firefox"
cp manifest.json.firefox manifest.json
if [[ "$1" == "local" ]];then
    npx web-ext build --ignore-files="assets/**" --overwrite-dest --filename="$name.local.xpi"
else
